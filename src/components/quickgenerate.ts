                    ...
                    clearInterval(interval);
                        ...
                        const image_map_r = generations?.map(async g => {
                            ...
                            try {
                                ...
                                // For videos, use a more explicit filename format and description
                                ...
                                try {
                                    ...
                                    const filename = isVideo ? `video_${g.id}${fileExtension}` : `${g.id}${fileExtension}`;
                                    console.log(`[DEBUG] Creating attachment with filename: ${filename}, isVideo: ${isVideo}`);
                                    const attachment = new AttachmentBuilder(centraReq.body, {
                                        name: filename,
                                        description: isVideo ? "Video generated by AI" : "Image generated by AI"
                                    });
                        ...
                        if (contentType === "video" && files.length > 0) {
                            console.log(`[DEBUG] Using video-optimized response`);
