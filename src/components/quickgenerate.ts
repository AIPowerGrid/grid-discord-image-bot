                    ...
                    if (ctx.client.config.advanced?.result_structure_v2_enabled ?? true) {
                        ...
                        const image_map_r = images.generations?.map(async (g, i) => {
                            ...
                            const embed = new EmbedBuilder({
                                ...
                                image: {url: `attachment://${g.seed ?? `${contentType.toLowerCase()}${i}`}.webp`},
                                color: Colors.Blue,
