...
${showEta ? `**ETA:** <t:${Math.floor(Date.now()/1000)+(status?.wait_time ?? 0)}:R>` : "**Searching for workers...**"}`
        ...
        async function getCheckAndDisplayResult(precheck?: boolean) {
            ...
            // Regular status check or video fallback check
            if (!status.done) {
                // For non-video channels or when not ready, just return status
                if (!isVideoChannel || (!(status?.finished && status.finished > 0) && status?.processing !== 0)) {
            ...
            // When we get here, we're processing the completed generation
            done = true;
            ...
            if(ctx.client.config.advanced?.result_structure_v2_enabled ?? true) {
                ...
                const image_map_r = generations?.map(async g => {
                    try {
                        ...
                        console.log(`[DEBUG] Content detected as ${mediaType}, isVideo=${isVideo}, response=${isVideoResponse}, hasVideoFilename=${hasVideoFilename}`);
                        ...
                        if (g.censored) {
                            ...
                            if (parts.length > 1) {
                                let buffer = Buffer.from(parts[1] as string, 'base64');
                                ...
                                const filename = isVideo ? `video_${g.id}${fileExtension}` : `${g.id}${fileExtension}`;
                                
                                console.log(`[DEBUG] Creating attachment with filename: ${filename}, isVideo: ${isVideo}`);
                                attachment = new AttachmentBuilder(buffer, {
                                    name: filename,
                                    description: isVideo ? "Video generated by AI" : "Image generated by AI"
                                });
                            } else {
                            ...
                            try {
                                let buffer = Buffer.from(g.img, 'base64');
                                const isAnimated = isAnimatedWebp(buffer);
                                console.log(`[DEBUG] WebP animation check: isAnimated=${isAnimated}`);
                    ...
                    if (g.img.startsWith('data:')) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                attachment = new AttachmentBuilder(Buffer.from(arrayBuffer), {name: `${g.seed ?? `${mediaType.toLowerCase()}${i}`}${fileExtension}`});
                            } catch (fetchError) {
