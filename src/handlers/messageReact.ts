            ...
            if(res?.rowCount) {
                await target_user.send({
                    embeds: [{
                        ...
                        description: `**${u.displayName}** tried to gifted you **${emoji.amount ?? 1}** Kudos on [this message](${r.message.url}).${emoji.message ? `\n${emoji.message}` : ""}\n\nSince you are not logged in you **did not** receive them. Log in with your [ai horde account](https://api.aipowergrid.io/register) within a week to claim your Kudos.`,
                        color: Colors.Red
                    }]
                }).catch(console.error)
                await u.send({
